...
add_input (const char *filename, char ***package_names, size_t *n_package_names)
{
  ...
  if (!*package_names) {
    *package_names = xmalloc (sizeof (char **) * 2);
    *n_package_names = 0;
  }

  if (read_file (filename, &orig_input_buffer) == 0)
    return;

  input_buffer = orig_input_buffer;
  while (input_buffer && (position_of_newline = strchrnul (input_buffer, '\n')))

    {
      size_t sz = position_of_newline - input_buffer;
      char *new = xmalloc (sz+1);
      strncpy (new, input_buffer, sz);
      new[sz] = '\0';

      *package_names = xrealloc (*package_names,
				 sizeof (char **) * (*n_package_names + 1));
      (*package_names)[*n_package_names] = new;
      (*n_package_names)++;
