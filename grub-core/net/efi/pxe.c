...
pxe_configure (struct grub_efi_net_device *dev, int prefer_ip6)
{
  ...
  if (mode->using_ipv6)
	  ...
	  grub_memcmp (manual_address->address, mode->station_ip.v6, sizeof (manual_address->address)) != 0)
	{
	  ...
	  grub_memcpy (station_ip.v6, manual_address->address, sizeof (station_ip.v6));
	  ...
	  grub_memcmp (manual_address->address, mode->station_ip.v4, sizeof (manual_address->address)) != 0)
	{
	  ...
	  grub_memcpy (station_ip.v4, manual_address->address, sizeof (station_ip.v4));
	  grub_memcpy (subnet_mask.v4, manual_address->subnet_mask, sizeof (subnet_mask.v4));
  ...
  if (mode->using_ipv6)
	...
	mode->station_ip.v6[0],
	mode->station_ip.v6[1],
	mode->station_ip.v6[2],
	mode->station_ip.v6[3],
	mode->station_ip.v6[4],
	mode->station_ip.v6[5],
	mode->station_ip.v6[6],
	mode->station_ip.v6[7],
	mode->station_ip.v6[8],
	mode->station_ip.v6[9],
	mode->station_ip.v6[10],
	mode->station_ip.v6[11],
	mode->station_ip.v6[12],
	mode->station_ip.v6[13],
	mode->station_ip.v6[14],
	mode->station_ip.v6[15]);
    }
  else
    {
	  ...
	  int type __attribute__((unused)))
{
  ...
  if (pxe->mode->using_ipv6)
	...
	grub_memcpy (server_ip.v6, ip6, sizeof (server_ip.v6));
	...
	server_ip.v4[i] = grub_strtoul (p, &p, 10);
    }
